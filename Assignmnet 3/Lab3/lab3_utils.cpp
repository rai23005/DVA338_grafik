...
//Switch  to next shader program in the  shader 
...
//To load a model from the file path
...
//Set material value for each surface one the object 
void SetObjMaterialUniforms(GLuint shaderProgram, obj* model) {
    ...
    //If the objekt have Abient, diffuse specular value and shines 
    // Ambient (Ka)
    ...
    //If object have a text maping (or not, if the shader use the mapping or the orther value frpm ambient, diffus and specular) 
...
//Prepare the model, what the model shall have for value to send in to shader 
...
//For a loop, that frame will reset with %  
...
//Get a vector that that calulate animate vector (how much the model have move from start) 
